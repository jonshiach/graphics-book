...
# 
# Side on view of the distance from the plane $p$ defined by the point $\mathbf{p}$ and normal vector $\mathbf{n}$ and the point $\mathbf{q}$.
# ```
# 
# The [geometric definition of the dot product](geometric-dot-product-definition) between the vectors $\mathbf{q} - \mathbf{p}$ and $\mathbf{n}$ is
# 
# ```{math}
