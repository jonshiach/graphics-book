...
# 	
# - **Clipped screen space**: Any objects in the screen space that are outside of the region that visible to the camera are removed and objects that lie partially outside are `clipped' to the edges. In addition, the surfaces that should not be visible to the viewer are removed. 
# 	
# - **Raster**: The colours of the individual pixels on a display screen are determined from the information obtained about the virtual environment up to this point and any lighting conditions used. This information is stored in a \textbf{raster array} where each element corresponds to a pixel on the display.
# 	
