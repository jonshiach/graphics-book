...
Euclid described a *point* as *"that which has no part"*. A point has position only, it has no length, width or thickness, and thus no area or volume. As we have [already seen](co-ordinate-systems-section), a typical point in $\mathbb{R}^n$ is described by its co-ordinates $(a_1,a_2,\dots,a_n)$, where the $a_i \in \mathbb{R}$.
...
:label: vector-equation-of-a-plane-definition
...
Recall the {prf:ref}`geometric definition of a dot product<geometric-dot-product-definition>` which states
...
:label: equation-of-a-sphere-definition
