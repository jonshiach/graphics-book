...
where $y$ and $x$ are the co-ordinates of a point on the line, $m = (x_{\min} - x_{\max}) / (y_{\min} - y_{\max})$ is the gradient of the line and $c$ is where the line intercepts the $y$-axis. Rearranging this to make $x$ the subject gives
